...
function generarActaTraspaso(data) {
  ...
  /*
   * La cuota se consulta ANTES de intentar el envio.
   *
   * Cuando se agota, MailApp deja de entregar. El mensaje de error que devuelve
   * no siempre es claro, y el efecto practico es el peor posible: la app decia
   * "enviada" y el correo no llegaba nunca. Preguntando primero, el inspector se
   * entera en el momento y puede bajar el acta de Drive y mandarla a mano.
   */
  var cuota = MailApp.getRemainingDailyQuota();
  if (cuota < lista.validos.length) {
    resultado.status = "parcial";
    resultado.message = "El acta se generó y quedó en Drive, pero NO se envió: " +
      "se agotó la cuota diaria de correos de Google (quedan " + cuota + " para " +
      lista.validos.length + " destinatarios). Descargá el acta de Drive y envíala a mano. " +
      "La cuota se renueva en unas horas.";
    registrarEnvioCorreo(fecha, turno, lista.validos, false, resultado.message);
    return resultado;
  }
  ...
  try {
    MailApp.sendEmail({
      ...
      // Un remitente con nombre y con responder-a reduce que el correo termine
      // clasificado como spam, que era la otra explicacion de "dice que se envio
      // pero no llega".
      name: "App Calidad",
    ...
    resultado.cuotaRestante = MailApp.getRemainingDailyQuota();
    registrarEnvioCorreo(fecha, turno, lista.validos, true, "");
  } catch (err) {
    ...
    registrarEnvioCorreo(fecha, turno, lista.validos, false, err.message);
  }
...
/*
 * Deja constancia de cada intento de envio en la hoja "Log_Correos".
 *
 * Sin esto, un acta que no llega no deja rastro en ningun lado: no hay forma de
 * saber si se intento mandar, a quien, ni que respondio Google. Con el log, la
 * pregunta "se envio o no?" se contesta mirando la planilla.
 */
function registrarEnvioCorreo(fecha, turno, destinatarios, ok, detalle) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var hoja = ss.getSheetByName("Log_Correos");
    if (!hoja) {
      hoja = ss.insertSheet("Log_Correos");
      hoja.appendRow(["fecha_hora", "fecha_turno", "turno", "destinatarios",
                      "resultado", "detalle", "cuota_restante"]);
      hoja.setFrozenRows(1);
    }
    hoja.appendRow([
      Utilities.formatDate(new Date(), "America/Santiago", "yyyy-MM-dd HH:mm:ss"),
      fecha, turno, (destinatarios || []).join(", "),
      ok ? "ENVIADO" : "FALLO", detalle || "",
      MailApp.getRemainingDailyQuota()
    ]);
  } catch (err) {
    // Nunca hacer fallar el acta por no poder escribir el log.
    Logger.log("No se pudo registrar el envio: " + err);
  }
}
...
/*
 * ==========================================================================
 * ESCRITURA SEGURA DE MEDICIONES
 * ==========================================================================
 *
 * El problema: Google Sheets, con la configuracion regional de Chile, lee el
 * punto como separador de FECHA. Una medicion de "1.6" mm se guarda como 1 de
 * junio, "13.8" como 13 de agosto y "5.5" como 5 de mayo. En la planilla quedan
 * valores tipo 2026-06-01T04:00:00.000Z donde deberia haber un numero, y el PDF
 * de esa ronda certifica una fecha en vez de una medida.
 *
 * Por que pasaba: setValues escribe con el formato que la celda ya tenga. Si es
 * "automatico", Sheets interpreta el contenido. Forzando el formato a texto (@)
 * ANTES de escribir, el valor entra tal cual lo tecleo el inspector.
 *
 * Columnas 17 a 29 de Inspecciones_Detalle: las 13 mediciones numericas.
 */
var COL_MEDICIONES_INICIO = 17;
var COL_MEDICIONES_CANT = 13;

function escribirDetalleSeguro(hoja, filaInicio, filas) {
  if (!filas || !filas.length) return;

  // El formato va primero: despues de setValues, Sheets ya convirtio el valor.
  hoja.getRange(filaInicio, COL_MEDICIONES_INICIO, filas.length, COL_MEDICIONES_CANT)
      .setNumberFormat("@");

  hoja.getRange(filaInicio, 1, filas.length, filas[0].length).setValues(filas);
}


/*
 * Deja la hoja preparada para que las filas NUEVAS ya nazcan como texto.
 * Ejecutar UNA vez desde el editor. Es independiente de la reparacion del
 * historico: esto evita el problema hacia adelante.
 */
function blindarColumnasMediciones() {
...
/*
 * Repara el historico ya corrompido.
 *
 * Como se revierte: Sheets guardo "13.8" como el 13 de agosto. La conversion es
 * deterministica en sentido inverso -> dia + "." + mes. Solo se tocan celdas que
 * hoy contienen una fecha; cualquier cosa que ya sea texto o numero se respeta.
 *
 * ANTES DE EJECUTAR: sacar una copia de la planilla (Archivo > Hacer una copia).
...
/*
 * DIAGNOSTICO - ejecutar a mano desde el editor cuando un acta "se envia" y no llega.
 *
 * Responde las cuatro preguntas que no se pueden contestar desde el telefono:
 * cuantos correos quedan hoy, desde que cuenta salen, a quienes se manda, y si
 * un envio de prueba llega de verdad.
 */
function diagnosticoCorreo() {
  var cuota = MailApp.getRemainingDailyQuota();
  var cuenta = Session.getEffectiveUser().getEmail();
  var cfg = leerConfigActa();

  Logger.log("Correos disponibles hoy: " + cuota);
  Logger.log("Los correos salen desde: " + cuenta);
  Logger.log("Destinatarios guardados: " + (cfg.destinatarios.length
    ? cfg.destinatarios.join(", ") : "NINGUNO - por eso el acta no se envia a nadie"));

  if (cuota === 0) {
    Logger.log("PROBLEMA: cuota agotada. Se renueva en unas horas.");
    return;
  }
  if (!cfg.destinatarios.length) {
    Logger.log("PROBLEMA: no hay destinatarios guardados. Escribilos en el modal del acta.");
    return;
  }
  ...
  try {
    MailApp.sendEmail({
      ...
      name: "App Calidad",
      replyTo: cuenta,
      subject: "PRUEBA · App Calidad · " + Utilities.formatDate(new Date(), "America/Santiago", "yyyy-MM-dd HH:mm"),
      htmlBody: "<p>Correo de prueba de App Calidad.</p>" +
                "<p>Si lo estás leyendo, el envío funciona y el problema del acta " +
                "está en otro lado. Si no llegó, revisá la carpeta de spam.</p>"
    });
    Logger.log("Prueba enviada a: " + cfg.destinatarios.join(", "));
    Logger.log("Revisar bandeja Y carpeta de spam en los proximos minutos.");
  } catch (err) {
    Logger.log("FALLO el envio de prueba: " + err.message);
  }
}
...
function crearRonda(data) {
  ...
  escribirDetalleSeguro(sheetDet, lastRowDet + 1, filas);
...
function cerrarRonda(data) {
  ...
  /*
   * Una fila "Detenida" NO es una ronda: es el registro del paro. No tiene
   * mediciones que cerrar. Que el laboratorio llegara hasta aca significaba que
   * la app estaba apuntando a una fila equivocada, normalmente por un id de envio
   * viejo reusado en otro turno. El mensaje dice que hacer, no solo que fallo.
   */
  if (estadoActual === "Detenida") {
    return {
      status: "error",
      message: "El registro " + data.idRonda + " es una DETENCIÓN de máquina, no una ronda de inspección, " +
               "así que no tiene mediciones que cerrar. Cerrá sesión y volvé a entrar: eso descarta el " +
               "registro que quedó abierto. Después inicia una ronda nueva desde terreno."
    };
  }
  ...
  /*
   * Las cinco columnas del cierre se escriben en UNA sola llamada.
   *
   * Cada setValue es un viaje de ida y vuelta a Sheets con su costo fijo. Eran
   * cinco viajes seguidos para escribir cinco celdas contiguas de la misma fila;
   * un setValues sobre el rango K:O hace lo mismo en uno.
   */
  sheetCab.getRange(fila, 11, 1, 5).setValues([[
    dataParaPdf.observaciones,      // K  observaciones de laboratorio
    pdfUrl,                         // L  PDF individual
    "Cerrada",                      // M  estado
    data.inspector_lab || "",       // N  quien ensayo
    infoCierre.fechaHoraCompleta    // O  fecha y hora de cierre
  ]]);
...
function sincronizarDetalleLaboratorio(sheetDet, idRonda, cavidadesLab, datosCabecera) {
  /*
   * Antes se traia "Inspecciones_Detalle" entera para trabajar sobre las 1 a 24
   * filas de UNA ronda. Esa hoja crece una fila por cavidad, asi que es la que
   * mas pesa de toda la planilla: a mil rondas son decenas de miles de filas de
   * 29 columnas viajando en cada cierre de laboratorio.
   *
   * Ahora se barre una sola columna para ubicar las filas y recien despues se
   * leen completas las que hacen falta. `datos` se indexa igual que antes (por
   * numero de fila - 1) para no tocar el resto de la funcion.
   */
  var ultimaDet = sheetDet.getLastRow();
  var datos = [];
  var indicesRonda = [];
  var indicePorCavidad = {};

  if (ultimaDet >= 2) {
    var idsDet = sheetDet.getRange(2, 2, ultimaDet - 1, 1).getValues();
    var buscado = String(idRonda);
    var filasReales = [];

    for (var q = 0; q < idsDet.length; q++) {
      if (String(idsDet[q][0]) === buscado) filasReales.push(q + 2);
    }

    if (filasReales.length > 0) {
      var ancho = sheetDet.getLastColumn();
      var mapa = leerFilasDispersas(sheetDet, filasReales, ancho);
      for (var w = 0; w < filasReales.length; w++) {
        var nf = filasReales[w];
        datos[nf - 1] = mapa[nf];
        var nro = Number(mapa[nf][2]) || 1;
        indicePorCavidad[nro] = nf - 1;
        indicesRonda.push(nf - 1);
      }
    }
  }
  ...
  if (indicesRonda.length === 0) {
    ...
    escribirDetalleSeguro(sheetDet, lastRow + 1, nuevas);
  ...
  if (maxIdx - minIdx + 1 === indicesRonda.length) {
    ...
    escribirDetalleSeguro(sheetDet, minIdx + 1, bloque);
  } else {
    for (var r = 0; r < indicesRonda.length; r++) {
      var ir = indicesRonda[r];
      escribirDetalleSeguro(sheetDet, ir + 1, [datos[ir]]);
    }
  }
  ...
  if (extras.length > 0) {
    ...
    escribirDetalleSeguro(sheetDet, lastExtra + 1, filasExtra);
  }
...
function eliminarDetallePorRonda(sheetDet, idRonda) {
  var ultima = sheetDet.getLastRow();
  if (ultima < 2) return 0;

  // Columna B para ubicar; borrar sigue siendo fila por fila porque deleteRow
  // no acepta rangos dispersos.
  var ids = sheetDet.getRange(2, 2, ultima - 1, 1).getValues();
  var buscado = String(idRonda);
  var borradas = 0;

  // De abajo hacia arriba: borrar filas desplaza los indices siguientes.
  for (var i = ids.length - 1; i >= 0; i--) {
    if (String(ids[i][0]) === buscado) {
...
function registrarDetencion(data) {
  ...
  sheetCab.appendRow([
    ...
    data.accion_correctiva || "",   // Z
    data.responsable || "",         // AA
    // AB - Observaciones de terreno. En una detencion la columna K ya lleva la
...
function contarDetallePorRonda(sheetDet, idRonda) {
  ...
  // Columna B (idInspeccion) y nada mas: contar no necesita las 29 columnas.
  var ids = sheetDet.getRange(2, 2, ultima - 1, 1).getValues();
  var buscado = String(idRonda);
  var total = 0;
  for (var i = 0; i < ids.length; i++) {
    if (String(ids[i][0]) === buscado) total++;
  }
  return total;
}
...
function encontrarFilaPorId(sheet, id, col0) {
  ...
  // Una sola columna. Antes se traia la hoja entera para comparar un identificador.
  var ids = sheet.getRange(2, col0 + 1, ultima - 1, 1).getValues();
  var buscado = String(id);

  // De atras hacia adelante: lo que se busca casi siempre es reciente.
  for (var i = ids.length - 1; i >= 0; i--) {
...
/*
 * ==========================================================================
 * TURNO OPERATIVO SIN Utilities.formatDate
 * ==========================================================================
 *
 * getInfoTurnoOperativo es correcta pero cara: llama a Utilities.formatDate
 * cuatro veces. Eso no importa una vez, pero contarRondasMaquinaTurno la
 * llamaba POR CADA FILA de la planilla. Con 650 rondas acumuladas son mas de
 * 2600 llamadas en cada guardado, y el costo crece todos los dias.
 *
 * La columna C guarda texto "yyyy-MM-dd HH:mm:ss" escrito por este mismo
 * script, asi que la hora y la fecha se leen por posicion, sin formatear nada.
 * Si la celda no tiene ese formato (alguien la edito a mano y quedo un Date),
 * se cae a la funcion completa: correccion antes que velocidad.
 */
function infoTurnoRapido(valor) {
  var s = String(valor === null || valor === undefined ? "" : valor);

  // "2026-09-17 14:22:05" -> fecha en 0..9, hora en 11..12
  if (s.length >= 13 && s.charAt(4) === "-" && s.charAt(7) === "-" && s.charAt(10) === " ") {
    var hora = Number(s.substr(11, 2));
    if (!isNaN(hora)) {
      var iso = s.substr(0, 10);
      return {
        turno: (hora >= 8 && hora < 20) ? "Turno Mañana" : "Turno Noche",
        fechaOperativa: (hora < 8) ? sumarDiasISO(iso, -1) : iso
      };
    }
  }

  var completo = getInfoTurnoOperativo(valor);
  return { turno: completo.turno, fechaOperativa: completo.fechaOperativa };
}
...
function contarRondasMaquinaTurno(sheetCab, maquinaId, fechaOperativa, turno) {
  ...
  /*
   * Solo tres columnas, no las 28. Traer la hoja completa para mirar tres
   * valores multiplica por diez los datos que viajan desde Sheets, y esta
   * funcion corre en cada guardado de ronda.
   *
   * Se piden en un solo getValues (C hasta M) porque cada llamada a getRange
   * tiene costo fijo: una lectura ancha sale mas barata que tres angostas.
   */
  var bloque = sheetCab.getRange(2, 3, ultima - 1, 11).getValues(); // C..M
  var count = 0;

  for (var i = 0; i < bloque.length; i++) {
    var fechaHora = bloque[i][0];        // C
    if (!fechaHora) continue;
    if (String(bloque[i][3]) !== String(maquinaId)) continue;   // F, lo mas selectivo primero

    var estado = String(bloque[i][10] || "").trim() || "Cerrada"; // M
    if (estado === "Cancelada") continue;

    var info = infoTurnoRapido(fechaHora);
    if (info.fechaOperativa === fechaOperativa && info.turno === turno) count++;
  }
  return count;
}
...
function generarPDFsPorMaquinaTurno(turnoParam, fechaParam) {
  ...
  for (var maquinaKey in registrosPorMaquina) {
    ...
    /*
     * Una maquina que solo tiene detenciones no tiene nada que consolidar: el PDF
     * saldria con la matriz de evolucion vacia. Se omite.
     *
     * No se pierde informacion: las detenciones son justamente lo que el acta de
     * traspaso reporta, con su hora, motivo y responsable. Y cada PDF que no se
     * genera son varios segundos menos de espera, que es lo que hacia que cerrar
     * turno se pasara del tiempo maximo del navegador.
     */
    var tieneRondas = false;
    for (var z = 0; z < listaMaq.length; z++) {
      if (!listaMaq[z].esDetencion) { tieneRondas = true; break; }
    }
    if (!tieneRondas) { saltadas++; continue; }
...
function consolidadoMaquinaHTML(maquina, lista, turno, fecha, specsPorProducto) {
  ...
  for (var k = 0; k < rondas.length; k++) {
    ...
    // Los comentarios van separados y rotulados: quien lee el informe necesita
    // saber si una observacion la escribio el inspector en maquina o el
    // laboratorio horas despues.
    if (String(ro.observacionesTerreno || "").trim()) {
      detalleRondas += '<div class="obs obs-t"><b>Observaciones de terreno:</b> ' +
        escaparHTML(ro.observacionesTerreno) + '</div>';
    }
    if (String(ro.observaciones || "").trim()) {
      detalleRondas += '<div class="obs obs-l"><b>Observaciones de laboratorio:</b> ' +
        escaparHTML(ro.observaciones) + '</div>';
    }
    detalleRondas += '</div>';
  }
  ...
  return '' +
    '<html><head><meta charset="UTF-8"><style>' +
      ...
      '.obs { padding:4px 7px; font-size:9px; margin-top:3px; }' +
      '.obs-t { background:#eff6ff; border-left:3px solid #0284c7; }' +
      '.obs-l { background:#f5f3ff; border-left:3px solid #7c3aed; }' +
