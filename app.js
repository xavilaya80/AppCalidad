 ...
 *
 * Ese automatismo se RETIRO a pedido del area de calidad.
 *
 * Motivo: la especificacion de Productos_Specs es un valor de referencia, no un
 * limite duro. Una medida puede quedar apenas por encima o por debajo y seguir
 * siendo conforme segun el criterio del inspector, que es quien tiene delante la
 * pieza. Marcar "No Cumple" sola convertia esa referencia en un limite y obligaba
 * a corregir el boton a mano en rondas que estaban bien.
 *
 * Comportamiento actual:
 *     - El inspector escribe el valor.
 *     - El boton queda en "Cumple" y NADIE lo cambia automaticamente.
 *     - Si la pieza no cumple, el inspector toca el boton.
 *
 * Lo que SI se conserva es el rango escrito al lado de cada campo (ver
 * refrescarValidacion): es informacion de referencia para decidir, no un veredicto.
...
/*
 * Antes esta funcion comparaba el valor contra el rango y marcaba el boton.
 * Ahora solo limpia el resaltado del campo: el veredicto lo pone el inspector.
 *
 * Se conserva la funcion en vez de borrar sus tres llamadas porque sigue siendo
 * el punto donde limpiar el pintado que pudo quedar de una version anterior
 * cacheada en el telefono.
 */
function evaluarMedicion(input) {
  if (!input || !input.closest) return null;
  const fila = input.closest('.medicion-row');
...
function filaMedicionHTML(prefijo, c, campo, datos) {
  ...
  return `<div class="medicion-row">
    <label for="${idMed}">${campo.label}</label>
...
function resetFormularioIdentificacion() {
  ...
  const obsTerr = document.getElementById('input-observaciones-terreno');
  if (obsTerr) obsTerr.value = '';
  // El bloque de solo lectura pertenece a la ronda que se acaba de cerrar.
  const bloqueObsT = document.getElementById('bloque-obs-terreno');
  if (bloqueObsT) bloqueObsT.style.display = 'none';
...
/*
 * El id reservado vale SOLO dentro del turno en que se acuño.
 *
 * De donde viene el problema: en una PWA instalada la pestaña no se cierra nunca,
 * asi que sessionStorage sobrevive dias. Un rechazo del backend conserva el id a
 * proposito, para que el reenvio corrija la fila en vez de duplicarla. Si el
 * inspector no reintenta y se va, ese id queda vivo indefinidamente.
 *
 * Que pasaba entonces: dias despues, al reactivar una maquina detenida, la ronda
 * nueva reusaba ese id viejo. crearRonda es idempotente por id: encontraba la
 * fila anterior, no escribia nada, y respondia con el correlativo y el turno de
 * ENTONCES. Para el inspector la app "volvia" al turno en que la maquina se
 * detuvo, y al cerrar esa ronda el backend la rechazaba.
 *
 * Por que se ata al turno y no a un plazo de horas: un plazo fijo puede cruzar el
 * cambio de turno de las 08:00 o las 20:00 y dejar pasar un id de la jornada
 * anterior. El turno operativo es el mismo criterio con el que el backend agrupa
 * las rondas, asi que si cambio el turno, el id no sirve. Punto.
 */
function idEnvioEstable(prefijo) {
  const ahora = infoTurnoOperativo(new Date());
  const selloActual = ahora.fechaOperativa + '|' + ahora.turno;

  const guardado = sessionStorage.getItem(CLAVE_ID_ENVIO);
  const selloGuardado = sessionStorage.getItem(CLAVE_ID_ENVIO + '_turno');

  if (guardado && selloGuardado === selloActual && guardado.indexOf(prefijo) === 0) {
    return guardado;
  }

  const nuevo = prefijo + Date.now();
  sessionStorage.setItem(CLAVE_ID_ENVIO, nuevo);
  sessionStorage.setItem(CLAVE_ID_ENVIO + '_turno', selloActual);
  return nuevo;
}
...
function liberarIdEnvio() {
  ...
  sessionStorage.removeItem(CLAVE_ID_ENVIO + '_turno');
}
...
window.cargarPendiente = function (id) {
  ...
  // Lo que anoto terreno se muestra como contexto, no como algo editable: es el
  // registro de otra persona en otro momento y no debe poder reescribirse aca.
  const bloqueObsT = document.getElementById('bloque-obs-terreno');
  const textoObsT = document.getElementById('texto-obs-terreno');
  if (bloqueObsT && textoObsT) {
    const obsT = (pendiente.observacionesTerreno || '').trim();
    textoObsT.textContent = obsT;
    bloqueObsT.style.display = obsT ? 'block' : 'none';
  }
...
function actualizarFichaProducto(prodId) {
  ...
  // Cambiar de producto ya no cambia ningun veredicto: no hay validacion
  // automatica. Se repinta igual para limpiar resaltados que hayan quedado.
  refrescarValidacion();
...
function setupEventListeners() {
    ...
    btnDetenida.addEventListener('click', async () => {
      ...
      try {
        const resultado = await enviarConCola({
          ...
          // El acta lee la accion correctiva y el responsable desde la fila de la
          // detencion Y desde la ronda que reactiva la maquina. Ninguno de los dos
          // los estaba enviando, asi que el acta siempre salia sin el detalle de
          // que se hizo ni quien lo hizo.
          accion_correctiva: (document.getElementById('input-accion-correctiva') || {}).value || '',
          responsable: (document.getElementById('input-responsable') || {}).value || '',
          // Si el inspector anoto algo en el campo de terreno antes de detener
          // la maquina, no tiene por que perderse.
          observaciones_terreno: (document.getElementById('input-observaciones-terreno') || {}).value || ''
        }, `Detención · ${maquinaId} · ${motivo}`);

        if (resultado.encolado) {
          liberarIdEnvio();
          alert(mensajeEncolado(`Detención de ${maquinaId} (${motivo})`, colaPendientes().length, resultado.reintentable, resultado.lento));
          resetFormularioIdentificacion();
          return;
        }
  ...
  document.getElementById('btn-iniciar-ronda').addEventListener('click', async () => {
    ...
    try {
      ...
      const resultado = await enviarConCola({
        ...
        responsable: dataIdent.responsable,
      ...
      if (resultado.encolado) {
        ...
        alert(mensajeEncolado(
          `Ronda de ${dataIdent.maquinaId} (${numCav} cavidades)`,
          colaPendientes().length, resultado.reintentable, resultado.lento));
        resetFormularioIdentificacion();
        return;
      }
      ...
      if (d.duplicado) {
        /*
         * Caso tipico: el primer envio SI llego al servidor, pero la respuesta se
         * perdio o tardo demasiado, la app encolo y el reintento encontro la
         * ronda ya escrita.
         *
         * Para el inspector eso no es un problema ni un error: su registro esta
         * guardado. El texto anterior ("ya estaba registrada") sonaba a que habia
         * hecho algo mal y venia despues de dos avisos de fallo, asi que la
         * secuencia completa parecia un desastre cuando en realidad todo salio
         * bien.
         */
        alert(`\u2705 Ronda guardada.\n\n` +
              `El primer envío sí había llegado, así que no se creó una copia.\n` +
              `Registro ${d.id} · Ronda N° ${d.correlativoMaquina} de esta máquina en el turno.`);
        resetFormularioIdentificacion();
        await loadCatalogData();
        return;
      }
  ...
  document.getElementById('btn-cerrar-ronda').addEventListener('click', async () => {
    ...
    try {
      ...
      if (resultado.encolado) {
        alert(mensajeEncolado(`Cierre de laboratorio de ${idCerrado}`, colaPendientes().length, resultado.reintentable, resultado.lento));
...
function setupToggles() {
  document.addEventListener('click', (e) => {
    ...
    // La marca manual se mantiene aunque ya no exista validacion automatica:
    // viaja en los datos y sobrevive a los repintados, que es lo que impide que
    // cambiar el numero de cavidades borre un "No Aplica" puesto a mano.
...
/*
 * Acciones que tardan mucho mas que una ronda normal.
 *
 * Cerrar un turno arma un PDF por maquina, cada uno con la matriz de evolucion y
 * el desglose por cavidad, y convierte cada HTML a PDF. Con una decena de
 * maquinas eso supera holgadamente los 25 segundos: el navegador abortaba, el
 * boton quedaba en "Generando..." y el inspector no sabia si el turno se habia
 * cerrado o no. El trabajo SI se completaba en el servidor; lo que fallaba era la
 * espera del navegador.
 *
 * Tres minutos cubre el caso real con margen y sigue por debajo del limite de
 * ejecucion de Apps Script, asi que si se alcanza este timeout es que algo se
 * colgo de verdad, no que la operacion es lenta.
 */
const TIMEOUT_LARGO_MS = 180000;
/*
 * cerrarRonda entra aca porque tambien genera un PDF: el informe individual de la
 * inspeccion, con la matriz por cavidad. Con 24 cavidades el HTML es grande y la
 * conversion a PDF se lleva varios segundos. Estaba con el limite corto de 25 s,
 * asi que un cierre normal de laboratorio podia abortarse solo por tardar.
 */
const ACCIONES_LARGAS = ['cerrarTurno', 'generarConsolidado', 'generarActa', 'cerrarRonda'];

function timeoutPara(accion) {
  return ACCIONES_LARGAS.indexOf(accion) !== -1 ? TIMEOUT_LARGO_MS : TIMEOUT_RED_MS;
}
...
async function enviarAlBackend(payload) {
  ...
  const limite = timeoutPara(payload && payload.action);
  // Sin timeout, una red de planta que no responde deja el boton bloqueado para siempre.
  const alarma = setTimeout(() => control.abort(), limite);
  ...
  try {
    ...
    /*
     * Agotar el tiempo de espera NO es lo mismo que no tener red, aunque los dos
     * casos terminen encolando el registro.
     *
     * Antes los dos devolvian sinRed:true, asi que la tablet mostraba "SIN
     * CONEXION" con el WiFi andando perfecto y el inspector se iba a buscar
     * señal que ya tenia. La bandera `lento` separa las dos situaciones para que
     * el mensaje diga lo que de verdad paso.
     */
    const expiro = !!(err && err.name === 'AbortError');
    return {
      ok: false,
      sinRed: !expiro,
      lento: expiro,
      reintentable: expiro,
      message: expiro
        ? `El servidor tardó más de ${limite / 1000} segundos en responder. ` +
          `Hay conexión, pero no se pudo confirmar si quedó guardado: se reintentará solo.`
...
/*
 * Cada cuanto reintenta la cola por su cuenta.
 *
 * Estaba en 60 s, y ese era el motivo de que sincronizar tardara varios minutos:
 * el inspector llegaba al laboratorio con señal y la app esperaba a que cayera el
 * proximo tick del minuto. Si ese intento agarraba la señal todavia debil, otro
 * minuto mas. 20 s reintenta lo suficientemente seguido sin castigar la bateria,
 * y el disparo por foco (ver setupSincronizacionOportuna) cubre el caso normal
 * antes de que el temporizador llegue a correr.
 */
const COLA_REINTENTO_MS = 20000;
 ...
 *
 * Las tres encolan el registro, pero lo que el inspector debe hacer es diferente
 * en cada una: sin señal hay que moverse, con el servidor lento o ocupado hay que
 * esperar donde uno esta. Antes las dos ultimas decian "SIN CONEXION" y mandaban
 * a la gente a buscar una señal que ya tenia.
 */
function mensajeEncolado(descripcion, total, ocupado, lento) {
  let cabecera, cola;

  if (lento) {
    cabecera = '⏳ EL SERVIDOR TARDÓ DEMASIADO';
    cola = 'Hay conexión. Se reintentará solo en unos segundos, no hace falta moverse.';
  } else if (ocupado) {
    cabecera = '⏳ SERVIDOR OCUPADO';
    cola = 'Se enviarán solos en cuanto el servidor se libere.';
  } else {
    cabecera = '⚠️ SIN CONEXIÓN';
    cola = 'Se enviarán solos cuando vuelva la señal.';
  }
...
function setupCola() {
  ...
  window.addEventListener('online', () => {
    ...
    /*
     * Si hay cola, SOLO se sincroniza: sincronizarCola ya recarga el catalogo al
     * final cuando envio algo.
     *
     * Antes se llamaba a loadCatalogData aqui Y al final de la sincronizacion, o
     * sea que al volver la señal se hacia la carga completa dos veces, una antes
     * y otra despues de mandar la cola. Esa carga es la peticion mas pesada de la
     * app -maquinas, productos con tolerancias, siete dias de historial con el
     * detalle por cavidad- y es la que convertia unos segundos de envio en varios
     * minutos de espera.
     */
    if (colaPendientes().length > 0) {
      sincronizarCola(false);
...
/*
 * Sincroniza cuando la app vuelve al frente.
 *
 * Por que hace falta ademas del evento 'online': navigator.onLine solo informa si
 * la tablet tiene ALGUNA red conectada, no si llega al servidor. En planta la
 * tablet sigue asociada al WiFi dentro de una zona sin cobertura real, asi que
 * para el navegador nunca estuvo offline y el evento 'online' no se dispara al
 * salir de esa zona. La cola quedaba esperando el temporizador.
 *
 * El foco, en cambio, es exactamente lo que ocurre en la practica: el inspector
 * camina de terreno al laboratorio, desbloquea la tablet y abre la app. Ese
 * momento es la mejor señal disponible de "probemos de nuevo".
 */
function setupSincronizacionOportuna() {
  let ultimoIntento = 0;
  const MIN_ENTRE_INTENTOS_MS = 5000;   // evita disparar en cada cambio de pestaña
  ...
  const intentar = () => {
    ...
    if (ahora - ultimoIntento < MIN_ENTRE_INTENTOS_MS) return;
    ultimoIntento = ahora;
...
async function ejecutarActa(enviar) {
  ...
  try {
    ...
    if (d.enviado) {
      // "Enviada" significa que Google acepto el mensaje, no que haya llegado a
      // la bandeja. Decirlo evita que un correo en spam pase por entregado.
      msg += `\n📧 Enviada a: ${d.destinatarios.join(', ')}`;
      msg += `\nSi no llega en unos minutos, revisá la carpeta de spam.`;
      if (typeof d.cuotaRestante === 'number' && d.cuotaRestante < 10) {
        msg += `\n⚠️ Quedan ${d.cuotaRestante} correos disponibles hoy.`;
      }
    } else if (d.message) {
      msg += `\n⚠️ ${d.message}`;
    }
